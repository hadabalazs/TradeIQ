...
const COURSE_DEFS = [
  {
    ...
    intro: "Commodity trading and IFRS reporting are usually learned separately, and the gap between them is where costly mistakes happen — a contract negotiated on commercial instinct that then produces an accounting treatment nobody expected. This course teaches both sides together: how physical commodity trades are actually structured, priced and financed, and how IFRS 15, IFRS 9, IAS 2, IFRS 16 and IAS 37 apply to them. Worked examples run on real African coal and sulphur trades rather than textbook abstractions.",
  ...
  },
  {
    ...
    intro: "Open Finance changes who controls banking data: customers can permission a third party to access their accounts, and banks have to build for that rather than around it. This course covers the whole picture — why the model exists, how the UAE's CBUAE framework and Nebras API Hub implement it, the API and security standards involved, and what consent and data protection actually require in practice. It ends on the commercial side: the use cases and business models the framework makes possible.",
